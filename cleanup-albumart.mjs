...
const podcastRows  = db.prepare('SELECT DISTINCT img   FROM podcast_feeds    WHERE img IS NOT NULL AND img NOT LIKE "http%" AND img NOT LIKE "https%"').all();
db.close();
...
// Also protect locally-cached podcast feed cover art (podcast-*.ext)
podcastRows.map(r => r.img).forEach(f => referenced.add(f));
