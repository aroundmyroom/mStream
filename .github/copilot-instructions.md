...
- **MANDATORY pre-commit changelog check**: Before EVERY `git commit`, run `grep "^## v" changes-fork-velvet.md | head -3` and confirm the CURRENT version (from `package.json`) appears as a `## vX.Y.Z-velvet` header in the changelog. If it is missing, add the entry immediately — do NOT commit without it. This check has caught missing entries for v6.13.1, v6.13.2, v6.13.3 in the past.
- **MANDATORY regression check before touching large shared files**: `webapp/app.js`, `src/api/artists-browse.js`, and `webapp/admin/index.js` are high-traffic files modified by many features. Before starting any new feature work that touches these files, run `git diff HEAD~1 HEAD -- <file>` to see what the PREVIOUS commit added. Scan for feature keywords (function names, API routes, i18n keys). If the previous commit added something to that file, you MUST verify it is still present AFTER your edits by grepping for it. This rule exists because v6.13.4 silently deleted the entire custom artist placeholder image feature (added in v6.13.3) from all three files — 386+163+415 lines net-removed — without anyone noticing until the user reported it missing. Then v6.13.5 tried to restore it but missed critical imports (`busboy`, `Readable`) plus 321 lines of backend logic (MBID, fanart, bio, `_parseTadbArtist`, `saveFanartImage`, `MIN_ARTIST_IMG_BYTES`, `enrich-tadb` endpoint), 124 lines of `viewRecent` density grid from app.js, artist profile fanart hero + metadata chips, `_deduplicateSimArtists`/`_primaryArtist`, letter restore navigation, and hydration monitoring displays. **After any restoration, run `node /home/mStream/test/regression-artist-placeholder.cjs` — all 29 checks must pass before committing.**
...
- **MANDATORY README VERIFICATION after every version bump**: run `grep -n "velvet:v\|Current version" README.md docs/docker.md` and confirm ALL three lines show the NEW version before committing. If any line still shows the old version, fix it immediately. This step must NEVER be skipped.
- **Proactive version-bump check**: When starting a new session, when wrapping up a multi-feature implementation, or when analyzing upstream commits, compare `package.json` version against `changes-fork-velvet.md` headers. Run: `node -e "console.log(require('./package.json').version)"` and `grep "^## v" changes-fork-velvet.md | head -5`. If the changelog has **2 or more staged entries** with a version number higher than `package.json`, proactively mention it — e.g. *"package.json is still at v6.12.3-velvet but v6.12.4 and v6.12.5 are staged in the changelog — ready to cut a release?"*. Do not wait for the user to ask.
